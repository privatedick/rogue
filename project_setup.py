loop.close()
'''
        self.builder.create_file(self.tests_dir / "conftest.py", pytest_content)

        test_content = f'''"""Basic functionality tests."""

import pytest
from {self.config.name} import __version__

def test_version():
    """Verify version is string."""
    assert isinstance(__version__, str)
'''
        self.builder.create_file(self.tests_dir / "unit" / "test_basic.py", test_content)
...
- `poetry run start`: Run the main application
- `poetry run test`: Run tests
- `poetry run test-cov`: Run tests with coverage
- `poetry run lint`: Run linting
- `poetry run format`: Format code
- `poetry run typecheck`: Run type checking

## AI-Assisted Development

This project includes tools for AI-assisted development using the Gemini API.
...
MIT
'''
        self.builder.create_file(self.project_dir / "README.md", readme_content)

    def _create_script_files(self) -> None:
        """Create utility script files."""
        check_all_content = '''"""Run all checks script."""
...
def run_check(command: str, description: str) -> bool:
    ...
    console.print(f"Running {description}...", style="bold blue")
    result = subprocess.run(command.split(), capture_output=True, text=True)
    
    if result.returncode == 0:
        console.print("✓ Passed", style="bold green")
        return True
    else:
...
def main():
    ...
    if failed:
        console.print(f"❌ Failed checks: {', '.join(failed)}", style="bold red")
        sys.exit(1)
    else:
...
'''
        self.builder.create_file(
            self.project_dir / "scripts" / "check_all.py",
            check_all_content
        )
...
.task_cache/
'''
        self.builder.create_file(self.project_dir / ".gitignore", content)
...
'''
        self.builder.create_file(self.project_dir / ".pre-commit-config.yaml", content)
